...
export class Core {
  ...
  /** マーカー注視点を初期化する */
  initLookAtMarker(scene: THREE.Scene, point: THREE.Vector3) {
  ...
  getControlledObject(x: number, y: number) {
    try {
  ...
  /** 注視点を更新する */
  updateLookAtPoint(position: Vector3) {
  ...
  constructor(appElement: HTMLDivElement) {
    ...
    // イベント通知者を作成
    this._eventDispatcher = new EventDispatcher<RenderEvent>();
    ...
    const cameraDelegate = new Camera(
      scene,
    ...
    renderer.setAnimationLoop(t => {
      ...
      if (this._cameraMixer) {
        ...
        const mixier: any = this._cameraMixer;
      ...
      if (this._lookAtMixier) {
        ...
        const mixier: any = this._lookAtMixier;
        if (mixier._actions[0].paused) {
          this._lookAtMixier = null;
        }
      }
  ...
  /** 移動アニメーションを適用する */
  applyMoveAnimation(
    mixier: THREE.AnimationMixer,
    fromPoint: THREE.Vector3,
    toPoint: THREE.Vector3,
    duration: number
  ) {
    const action = mixier.clipAction(
      new THREE.AnimationClip('Action', duration, [
  ...
  /** 移動する */
  move(stands: THREE.Vector3, lookAt: THREE.Vector3, duration: number) {
    // カメラの移動をアニメーションする
    ...
    this._lookAtMarker.position.set(
      currentLookAt.x,
      currentLookAt.y,
      currentLookAt.z
    );
    ...
    this._lookAtMixier = new THREE.AnimationMixer(
      new Proxy(this._lookAtMarker, {
        set(obj, property, value, receiver) {
          if (property == 'matrixWorldNeedsUpdate') {
            cameraDelegate.camera.lookAt(marker.position);
            cameraDelegate.controls.target.set(
              marker.position.x,
              marker.position.y,
              marker.position.z
            );
