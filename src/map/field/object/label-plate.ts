...
/** テキストラベルを管理するクラス */
export class LabelPlate {
  // 文字を表示するテキストラベル
  private _plane: THREE.Mesh;
  // 文字の後ろに表示する文字背景と同色のパネル
  private _basement: THREE.Mesh;
  ...
  convertFontSizeToGlSize(fontSize: number) {
    // 17ptをGL空間上の0.033とする
    return 0.033 * (fontSize / 17);
  }

  /** テキストラベルを作成する */
  createLabel(property: LabelPlateInterface) {
    // ラベルサイズを計算する
    const labelSize = LabelTexture.contextMeasureText(
      property.label,
      property.fontSize
    );
    // ラベルのアスペクト比
    ...
    geometry.setAttribute(
      ...
      new THREE.BufferAttribute(
        new Float32Array(
          this.rectangleToVertex(
            property.x1 + xOffset,
            property.x1 + height + xOffset,
            property.z1 + zOffset,
            property.z1 + width + zOffset,
            property.y + 0.0001
          )
        ),
        3
    ...
    geometry.setAttribute(
      'uv',
      new THREE.BufferAttribute(new Float32Array(this.convertToUV()), 2)
    );
    return new THREE.Mesh(geometry, material);
  }

  /** テキストラベルの背景パネルを作成する */
  createBasement(property: LabelPlateInterface) {
    // 四角形のパネルを作成する
    const geometry = new THREE.BufferGeometry();
    geometry.setAttribute(
      'position',
