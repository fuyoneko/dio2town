...
export class SpritePanel {
  ...
  /** 画像の描画エリアを作成する */
  imageArea(scene: THREE.Scene, base64Texture: string) {
    // 空の画像データを作成する
  ...
  /** 角丸の下の、ピンの矢印を描画する */
  pinBasement(scene: THREE.Scene, googleAnotation = true) {
    ...
    if (!this._loaded) {
      ...
      new THREE.TextureLoader().loadAsync(this._imageSrcFile).then(image => {
        ...
        if (this._image.material.map) {
          this._image.material.map?.dispose();
