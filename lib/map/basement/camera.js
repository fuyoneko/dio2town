...
const MapOrbitControls_1 = require("./threejs-custom/MapOrbitControls");
/** カメラの操作情報を管理するクラス */
class Camera {
    /**
     * コンストラクタ
     ...
     */
    constructor(scene, aspect, target) {
        // カメラを初期化
        ...
        this._latestPointerDown = new three_1.Vector2();
        // カメラコントローラー設定
        const orbitControls = new MapOrbitControls_1.MapOrbitControls(camera, target);
        ...
        this._eventDispatcher = new three_1.EventDispatcher();
        orbitControls.domElement.addEventListener('pointerdown', e => {
            const event = e;
            ...
            if (width >= 1.0 && height >= 1.0) {
                ...
                // -1.0 ~ 1.0の大きさに正規化する
                this._latestPointerDown.set((x / width) * 2 - 1.0, -(y / height) * 2 + 1.0);
            }
