...
export class FieldPlace extends FieldObjectBase {
    ...
    constructor(scene, data, polygon) {
        ...
        // オブジェクトの表示スペースを設定する
        this._objectSpace = new GeoRectangle(new GeoPoint(data.x[0], data.y[0]), new GeoPoint(data.x[1], data.y[1]));
        // 建物（扁平なタイルを表示する）
        const base = Plane.drawPlane(scene, this._objectSpace.p1, this._objectSpace.p2, 0.01, Color.lightGrey);
        if (data.invisible) {
            ...
            // 選択中ハイライトを設定する
            this.initOverlay(scene, new GeoPoint(data.x[0], data.y[0]), new GeoPoint(data.x[1], data.y[1]), 0.01 + 0.003);
            // ホバー中のホバー表示を設定する
    ...
    createSpliteLabel(data) {
        ...
        const labelAcept = labelInfo.actualBoundingBoxAscent >= 0.01
            ...
            : 1.0;
        // ラベルを作成する
        const texture = LabelTexture.createLabelCanvas(data.label, {
    ...
    createBuildingObject(scene, data, polygon) {
        ...
        const building = Plane.drawPlane(scene, new GeoPoint(data.x[0], data.y[0]), new GeoPoint(data.x[1], data.y[1]), 0.04, Color.lightGrey).plane;
